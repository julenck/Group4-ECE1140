"""
Provides block list, map display, details panel, and active trains table.
...
class HW_Display(ttk.Frame):
    ...
    def __init__(self, master: tk.Misc):
        ...
        # Switch control buttons live under the block picker
        ...
        # Right side: Map and details panel
        right = ttk.Frame(content_frame)
        
        # Frame to hold map image and details panel
        ...
        row("Switch", "switch")
        row("Switch Map", "switch_map", 24)  # Wider for switch map

        grid.pack(anchor="nw", fill="both", expand=True)

        right.pack(side="left", fill="both", expand=True)

        # Bottom: Active Trains table
        ...
        try:
            style = ttk.Style()
            style.configure("Treeview", background="#33363b", foreground="#f2f3f4", fieldbackground="#33363b", rowheight=25)
            style.configure("Treeview.Heading", background="#2a2d31", foreground="#f2f3f4", relief="flat")
            style.map("Treeview", background=[("selected", "#4e9af1")], foreground=[("selected", "#ffffff")])
                    ...
                    if float(cmd_speed) > 0 and float(cmd_auth) > 0:
                        ...
                        if yards_per_sec > 0:
                            ...
                            eta_str = time_module.strftime(
                                "%H:%M:%S", time_module.localtime(eta_time)
                            )
                except Exception:
                    pass
    ...
    def show_vital(self, emergency: bool, speed_mph: float,
                   authority_yards: int, light_states=None, gate_states=None,
                   switch_states=None, occupied_blocks=None,
                   active_plc: str = "") -> None:
    ...
    def select_block(self, block_id: str):
        ...
        # Select the given block ID in the listbox
        try:
