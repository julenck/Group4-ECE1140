...
print(
    os.path.exists(
        r"C:\Users\wunmi\OneDrive\Documents\Academics\Trains\Group4-ECE1140\Track_and_Train\Train_Model\train_model_ui.py"
    )
)
# Get absolute paths
...
class MainUI(tk.Tk):
    def __init__(self):
        ...
        self.track_left_container = ttk.Frame(self.content_frame)
        self.track_right_container = ttk.Frame(self.content_frame)
        self.train_container = ttk.Frame(self.content_frame)

        # Initialize the UI components
        self.track_ui = TrackModelUI(
            self.track_left_container, self.track_right_container
        )
    ...
    def toggle_track_left(self):
        if self.track_left_visible:
            self.track_left_container.grid_forget()
            self.btn_track_left.config(text="Show Track Left")
            self.track_left_visible = False
        else:
            self.track_left_container.grid(
    ...
    def toggle_track_right(self):
        if self.track_right_visible:
            self.track_right_container.grid_forget()
            self.btn_track_right.config(text="Show Track Right")
            self.track_right_visible = False
        else:
            self.track_right_container.grid(
    ...
    def toggle_train(self):
        if self.train_visible:
            ...
            self.train_container.grid(row=0, column=2, sticky="nsew", padx=2, pady=2)
