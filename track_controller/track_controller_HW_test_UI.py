...
class TestUI(tk.Frame):
    ...
    def _build_ui(self):
        ...
        out = ttk.LabelFrame(self, text="Outputs")
        out.grid(row=1, column=0, columnspan=6, sticky="nsew", **pad)

        ttk.Label(out, text="Commanded Speed (mph):").grid(row=0, column=0, sticky="w", padx=8, pady=6)
        self.m_speed = ttk.Label(out, text="0", font=("Consolas", 14, "bold"))
        self.m_speed.grid(row=0, column=1, sticky="e", padx=8, pady=6)

        ttk.Label(out, text="Commanded Authority (yards):").grid(row=1, column=0, sticky="w", padx=8, pady=6)
        ...
        ttk.Label(ctl, text="Suggested Speed (input mph):").grid(row=0, column=1, padx=6)
        self.e_speed = ttk.Entry(ctl, width=6)
        self.e_speed.insert(0, "0")
        self.e_speed.grid(row=0, column=2, padx=4)

        ttk.Label(ctl, text="Suggested Authority (input yards):").grid(row=0, column=3, padx=6)
        self.e_auth = ttk.Entry(ctl, width=8)
        self.e_auth.insert(0, "0")
        self.e_auth.grid(row=0, column=4, padx=4)

        ttk.Button(ctl, text="Simulate", command=self._apply_inputs).grid(row=0, column=5, padx=6)
    ...
    def _sync_assets(self):
        ...
        if a:
            ...
            self.m_gates.config(text="CLOSED", foreground="red")
        else:
            self.m_signal_lights.config(text="GREEN", foreground="green")
            self.m_gates.config(text="OPEN", foreground="green")
