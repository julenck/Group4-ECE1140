...
class TrackNetworkBuilder:
        ...
        for branch_point, bp_info in self.network.branch_points.items():
            for target in bp_info.targets:
                ...
                if target < branch_point:
                    # BACKWARD = LOOP
                    self.network.loops.append(Loop(
    ...
    def _build_paths_bfs(self):
        ...
        sorted_branches = sorted(branch_targets.items(), key=lambda x: x[1], reverse=True)
        ...
        for branch_point, target in sorted_branches:
            ...
            print(f"  Branch {path_id}: from {branch_point}→{target}, {len(branch_blocks)} blocks {branch_blocks}{reconnect_info}")
            path_id += 1
        ...
        for branch_path in sorted(self.network.paths, key=lambda p: p.from_block if p.from_block else 0):
            if branch_path.type == 'branch' and branch_path.from_block in ordered_blocks:
        ...
        print(f"  Main path: {len(main_blocks)} blocks (ends at {main_blocks[-1] if main_blocks else 'N/A'})")
        ...
        for path in self.network.paths:
            for block in path.blocks:
                if block in assigned:
                    print(f"⚠️  VALIDATION ERROR: Block {block} appears in multiple paths!")
                assigned.add(block)
            ...
            if path.type == 'branch' and len(path.blocks) > 1:
                for i in range(len(path.blocks) - 1):
                    if path.blocks[i+1] != path.blocks[i] + 1:
                        print(f"⚠️  VALIDATION ERROR: Branch {path.id} not sequential: {path.blocks}")
                        break
    ...
    def print_network_summary(self):
        ...
        for path in self.network.paths:
            ...
            if path.connects_to_path is not None:
                print(f"    └─ Reconnects to Path {path.connects_to_path} at block {path.connects_at_block}")
