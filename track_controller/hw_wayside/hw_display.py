...
class HW_Display(ttk.Frame):
    ...
    def show_vital(self, emergency: bool, speed_mph: float, authority_yards: int, light_states=None, gate_states=None, switch_states=None, occupied_blocks=None, active_plc: str = "") -> None:
        ...
        """
        try:
            bids = [str(bid) for bid, _ in (light_states or [])]
            # union with occupied blocks if provided
            if occupied_blocks:
                bids = sorted(set(bids) | set(str(x) for x in occupied_blocks))
            if bids:
                self.set_blocks(bids)
            # update status area
